...
func TestBuilder_TypedDecodeSuccess(t *testing.T) {
	...
	if tgt.Name != "Alice" || tgt.Age != 42 || tgt.Kind != "pro" || tgt.Flag != true {
		t.Fatalf("unexpected struct %+v", tgt)
	}
	if tgt.Alias == nil || *tgt.Alias != aliasVal {
		t.Fatalf("alias not set")
	}
}
...
func TestBuilder_TypedMissingRequired(t *testing.T) {
	...
	var tgt typedTarget
	dec := b.MustBindStruct(&tgt)
	if err := dec.Decode(map[string]any{"name": "Bob"}, nil); err == nil {
		t.Fatalf("expected missing required age error")
	}
}
...
func TestBuilder_TypedEnumViolation(t *testing.T) {
	...
	var tgt typedTarget
	dec := b.MustBindStruct(&tgt)
	if err := dec.Decode(map[string]any{"kind": "enterprise"}, nil); err == nil {
...
func TestBuilder_TypedNumericCoercion(t *testing.T) {
	...
	var tgt typedTarget
	dec := b.MustBindStruct(&tgt)
	if err := dec.Decode(map[string]any{"age": float64(7), "name": ""}, nil); err != nil {
		t.Fatalf("decode failed: %v", err)
	}
	if tgt.Age != 7 {
		t.Fatalf("expected age 7 got %d", tgt.Age)
	}
}
...
func TestBuilder_TypedNoPartialMutation(t *testing.T) {
	...
	if !reflect.ValueOf(tgt).IsZero() {
		t.Fatalf("tgt should remain zero on failure: %+v", tgt)
	}
}
...
func TestBuilder_TypedOptionalUnmappedProperty(t *testing.T) {
	...
	var tgt typedTarget
	dec := b.MustBindStruct(&tgt)
	if err := dec.Decode(map[string]any{"ghost": "value"}, nil); err != nil {
		t.Fatalf("decode failed: %v", err)
	}
}
...
func TestBuilder_TypedRequiredUnmappedFails(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("expected bind error for required unmapped property")
	}
}
